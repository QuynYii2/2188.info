                                ...
                                <div class="product-item bg-light mb-4">
                                    ...
                                    <div class="text-center py-4">
                                        <div class="h6 text-decoration-none text-truncate" data-toggle="modal"
                                             data-target="#exampleModal{{$product->id}}">{{$trans->translateText($product->name)}}</div>
                                            ...
                                            <div class="modal-body">
                                                <div class="card tabs-product row" id="id-tabs-product">
                                                    ...
                                                    <div class="product-content col-md-12 py-1" style="z-index: 88;">
                                                        @csrf
                                                        <h2 class="product-title">{{$trans->translateText($product->name)}}</h2>
                                                        <small class="text-warning">{{$trans->translateText($product->category->name)}}</small>
                                                        ...
                                                        <div class="row">
                                                            ...
                                                            @foreach($attributes as $attribute)
                                                                ...
                                                                <div class="col-sm-6 col-6">
                                                                    <label for="{{$att->name}}">{{$trans->translateText($att->name)}}</label>
                                                                    <select id="{{$att->name}}" name="{{$trans->translateText($att->name)}}"
                                                                            class="form-control">
                                                                        @foreach($arrayAtt as $data)
                                                                            @php
                                                                                $property = \App\Models\Properties::find($data);
                                                                            @endphp
                                                                            <option value="{{$data}}">{{$trans->translateText($property->name)}}</option>
                                                                        @endforeach
                                                                    </select>
                                                                </div>
                                                            @endforeach
